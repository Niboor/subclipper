...
class SubtitleScanner(threading.Thread):
    ...
    def update_video_status(self, video_id: str, status: VideoScanStatus, fail_reason: str | None = None):
        with sqlite3.connect(self.sqlite_path) as conn:
            ...
            for i, _ in enumerate(segments[:-1]):
                ...
                percent = self.get_progress(partial_path)
    ...
    def get_progress(self, path: Path) -> float:
        with sqlite3.connect(self.sqlite_path) as conn:
            cursor = conn.cursor()
    ...
    def _insert_subtitle(self, subtitle: Subtitle):
        with sqlite3.connect(self.sqlite_path) as conn:
            cursor = conn.cursor()
            cursor.execute('INSERT OR REPLACE INTO subtitles (subtitle_id, video_id, text, start, end) VALUES (?,?,?,?,?)', [subtitle.id, subtitle.video_id, subtitle.text, subtitle.start, subtitle.end])
            conn.commit()
...
class SubtitleIndexer:
    ...
    def get_scanning_progress(self, search_subpath: Path) -> Optional[float]:
        ...
        progress = SubtitleIndexer.scanner.get_progress(search_subpath)
        return progress
